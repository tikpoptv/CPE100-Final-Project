...
void saveToCSV(struct Product products[], int product_count) {
    ...
    fprintf(file, "วันที่นำเข้าสินค้า,ชื่อสินค้า,จำนวน\n");
    for (int i = 0; i < product_count; i++) {
        fprintf(file, "2023-11-21,%s,%d\n", products[i].name, products[i].quantity);
    }
    
    fclose(file);
    printf("บันทึกข้อมูลเรียบร้อยแล้ว\n");
}
        ...
        switch (choice) {
            ...
            case 2:
                ...
                if (selected_index > 0 && selected_index <= product_count) {
                    ...
                    printf("ต้องการบันทึกข้อมูลที่อัพเดทแล้วใช่หรือไม่? (1 = ใช่, 0 = ไม่ใช่): ");
                    int save_option;
                    scanf("%d", &save_option);
                    if (save_option == 1) {
                        saveToCSV(products, product_count);
                    }
                } else {
