#include<stdio.h>
#include<string.h>//
#include <stdlib.h>//include function about atoi,atof
...
void readdaily(struct Product *products, int *column) {
    ...
    *column = 0;
    while (fgets(line, MAX_LINE_LENGTH, file) != NULL)
    {
        ...
        int cnt =0;
        while (value != NULL) {
            if((cnt%5)==0) products[*column].time = atof(value);
            else if((cnt%5)==1) strcpy(products[*column].productID, value);
            else if((cnt%5)==2) strcpy(products[*column].name, value);
            else if((cnt%5)==3) products[*column].quantity = atof(value);
            else if((cnt%5)==4) products[*column].price = atoi(value);
            cnt++;
            value = strtok(NULL, ",");
        }
        (*column)++;
